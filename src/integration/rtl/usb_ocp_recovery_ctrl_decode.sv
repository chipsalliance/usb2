...
//
// Role:
//   Sit above the EP adapter (A1). A1 forwards ALL EP0 SETUP transactions;
//   this module inspects the 8-byte SETUP, filters out non-class-specific
//   requests (they remain handled by the existing USB IP enumeration path
//   and must NOT be stalled here), and for class-specific recovery commands
//   (bmRequestType[6:5]==2'b01, bRequest in 0x22..0x2F) drives an internal
//   register bus toward A3 (regs) / A4 (cms_fifo via A3). Malformed class
//   requests are STALL'd via ctrl_set_stall.
//
...
//     to reg bus starts at wValue (Sec 8.5: wValue conveys starting byte
//     offset within the command payload; 0 for normal use).
//=============================================================================
...
module usb_ocp_recovery_ctrl_decode (
  ...
  always_comb begin
    ...
    // interface filtering is left to A1 / standard enum path.
    wlength_s   = {setup_pkt[63:56], setup_pkt[55:48]};

    is_class_s  = (bmrt_s[6:5] == 2'b01);
    is_in_s     = bmrt_s[7];
    brq_valid_s = (brq_s >= 8'h22) && (brq_s <= 8'h2F);
  end
    ...
    unique case (state_q)
      ...
      S_IDLE: begin
        if (setup_pkt_vld) begin
          if (is_class_s) begin
            // Class-specific: this module owns it.
            cmd_d    = brq_s;
            offset_d = wvalue_s;
            length_d = wlength_s;
            if (!brq_valid_s) begin
              // Unsupported class bRequest -> STALL per Sec 8.5.
              state_d = S_STALL;
          ...
          // else: standard or vendor request -> do nothing, stays with
          // the existing USB enumeration path. No stall.
        end
      end
